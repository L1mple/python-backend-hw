...
os.environ["POSTGRES_DB"] = "shop_db_test"
os.environ["POSTGRES_USER"] = "user"
os.environ["POSTGRES_PASSWORD"] = "qwerty"
os.environ["POSTGRES_HOST"] = "localhost"
os.environ["POSTGRES_PORT"] = "5430"
...
def create_test_db_if_not_exists(
    ...
    conn.set_isolation_level(ISOLATION_LEVEL_AUTOCOMMIT)  # чтобы можно было создавать базу
    cur = conn.cursor()
...
def truncate_tables_if_exist(conn, tables: list[str]):
    cur = conn.cursor()
    for table in tables:
        cur.execute("""
            SELECT EXISTS (
        ...
        if exists:
            ...
            print(f"Table {table} does not exist, skipping truncate.")
    conn.commit()
    cur.close()

# ВАЖНО: вызываем создание базы до импорта app/TestClient
...
def clean_db():
    ...
    truncate_tables_if_exist(conn, ["cart_items", "carts", "items"])
    conn.commit()
    conn.close()
