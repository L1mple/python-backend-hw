from fastapi import APIRouter, status
from fastapi.responses import JSONResponse

from .db import db
from .schemas import CreateItem, PatchItem, UpdateItem

# Роутер товаров
router = APIRouter()


@router.post("/item")
async def create_item(item: CreateItem):
    item = db.create_item(item)
    return JSONResponse(
        content=item.model_dump(),
        status_code=status.HTTP_201_CREATED,
        headers={"location": f"/item/{item.id}"},
    )


@router.get("/item/{item_id}")
async def get_item_by_id(item_id: int):
    item = db.get_item_by_id(item_id)
    if item is None:
        return JSONResponse(content={}, status_code=status.HTTP_404_NOT_FOUND)
    else:
        return JSONResponse(content=item.model_dump(), status_code=status.HTTP_200_OK)


@router.put("/item/{item_id}")
async def update_item(item_id: int, item: UpdateItem):
    item = db.update_item(item_id, item)
    if item is None:
        return JSONResponse(content={}, status_code=status.HTTP_404_NOT_FOUND)
    else:
        return JSONResponse(content=item.model_dump(), status_code=status.HTTP_200_OK)


@router.patch("/item/{item_id}")
async def patch_item(item_id: int, item: PatchItem):
    item, code = db.patch_item(item_id, item)
    if code == 404:
        return JSONResponse(content={}, status_code=status.HTTP_404_NOT_FOUND)
    if code == 304:
        return JSONResponse(content={}, status_code=status.HTTP_304_NOT_MODIFIED)
    return JSONResponse(content=item.model_dump(), status_code=status.HTTP_200_OK)


@router.delete("/item/{item_id}")
async def delete_item(item_id: int):
    item = db.delete_item(item_id)
    if item is None:
        return JSONResponse(content={}, status_code=status.HTTP_404_NOT_FOUND)
    else:
        return JSONResponse(content=item.model_dump(), status_code=status.HTTP_200_OK)


@router.get("/item")
async def get_items(
    offset: int = 0,
    limit: int = 10,
    min_price: float = None,
    max_price: float = None,
    show_deleted: bool = False,
):
    items, code = db.get_items(offset, limit, min_price, max_price, show_deleted)
    if code == 422:
        return JSONResponse(content=[], status_code=status.HTTP_422_UNPROCESSABLE_CONTENT)
    return JSONResponse(content=[item.model_dump() for item in items], status_code=status.HTTP_200_OK)
