...
def test_put_item_not_found():
    response = client.put('/item/999999', json={'name': 'ghost', 'price': 100.0})
    assert response.status_code == HTTPStatus.NOT_FOUND
...
def test_patch_item_deleted(existing_item):
    # удаляем и пробуем патчить
    item_id = existing_item['id']
    client.delete(f'/item/{item_id}')
    response = client.patch(f'/item/{item_id}', json={'price': 123})
    assert response.status_code == HTTPStatus.NOT_MODIFIED


def test_add_item_to_cart_not_found_cart(existing_item):
    response = client.post(f'/cart/999999/add/{existing_item["id"]}')
    assert response.status_code == HTTPStatus.NOT_FOUND


def test_add_item_to_cart_not_found_item(existing_empty_cart_id):
    response = client.post(f'/cart/{existing_empty_cart_id}/add/999999')
    assert response.status_code == HTTPStatus.NOT_FOUND


def test_add_item_to_cart_deleted_item(existing_empty_cart_id, existing_item):
    # удаляем товар и пробуем добавить
    client.delete(f'/item/{existing_item["id"]}')
    response = client.post(f'/cart/{existing_empty_cart_id}/add/{existing_item["id"]}')
    assert response.status_code == HTTPStatus.NOT_FOUND


def test_add_item_to_cart_existing(existing_not_empty_cart_id, existing_items):
    item_id = existing_items[0]
    # добавляем один товар дважды
    client.post(f'/cart/{existing_not_empty_cart_id}/add/{item_id}')
    response = client.post(f'/cart/{existing_not_empty_cart_id}/add/{item_id}')
    assert response.status_code == HTTPStatus.OK
    cart = response.json()
    quantities = [i['quantity'] for i in cart['items'] if i['id'] == item_id]
    assert all(q >= 1 for q in quantities)


def test_list_carts_with_quantity_filters(existing_not_empty_carts):
    response = client.get('/cart', params={'min_quantity': 0, 'max_quantity': 10})
    assert response.status_code == HTTPStatus.OK
    for cart in response.json():
        total_q = sum(item['quantity'] for item in cart['items'])
        assert 0 <= total_q <= 10
