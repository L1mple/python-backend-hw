...
def add(cart_id: int, item_entity, db: Session) -> CartEntity:
    ...
    if not db_cart:
        return None
    existing_item = db.query(CartItemDB).filter(CartItemDB.cart_id == cart_id, CartItemDB.item_id == item_entity.id).first()
    if existing_item:
...
def get_one(id: int, db: Session) -> CartEntity | None:
    ...
    for cart_item in db_cart.items:
        ...
        cart_items.append(CartItemInfo(id=item.id, name=item.name, quantity=cart_item.quantity, available=not item.deleted))
    return CartEntity(id=db_cart.id, info=CartInfo(items=cart_items, price=total_price))

def get_many(db: Session, offset: int = 0, limit: int = 10, min_price: float = None, max_price: float = None, min_quantity: int = None, max_quantity: int = None) -> Iterable[CartEntity]:
    db_carts = db.query(CartDB).offset(offset).limit(limit).all()
    for db_cart in db_carts:
        cart_entity = get_one(db_cart.id, db)
        if not cart_entity:
            continue
        if min_price is not None and cart_entity.info.price < min_price:
            continue
        if max_price is not None and cart_entity.info.price > max_price:
            continue
