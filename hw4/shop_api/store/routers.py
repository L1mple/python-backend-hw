...
from shop_api.store.services import ItemService, CartService
from shop_api.db.db_init import get_db
import json
...
def get_carts(
    ...
    if offset < 0 or limit <= 0:
        raise HTTPException(status_code=422, detail="Invalid offset or limit")
    service = CartService(db)
...
def demo_isolation(
    ...
    raise HTTPException(status_code=400, detail="isolation_type: dirty | non_repeatable | phantom")
